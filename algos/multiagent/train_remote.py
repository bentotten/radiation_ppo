...
class train_PPO:
    ...
    def train(self) -> None:
        ...
        agent_thoughts: Dict[int, RADCNN_core.ActionChoice] = {
            id: None for id in self.agents
        }
        ...
        for epoch in range(self.total_epochs):
            ...
            for steps_in_epoch in range(self.steps_per_epoch):
                ...
                for id, ac in self.agents.items():
                    agent_thoughts[id], heatmaps = ray.get(
                        ac.step.remote(
                ...
                if episode_reset_next_step:
                    self.process_render(
                        epoch_ended=epoch_ended, epoch=epoch, logger=self.loggers
                    )
                    ...
                    if timeout or epoch_ended:
                        ...
                        # used in that place.
                        for id, ac in self.agents.items():
                            bootstrap_results, _ = ray.get(
                                ac.step.remote(observations, hiddens=hiddens[id])
                            )
                    ...
                    for id, ac in self.agents.items():
                        ray.get(
                            ac.GAE_advantage_and_rewardsToGO.remote(last_state_value)
                        )
                    ...
                    if episode_over:
                        for id, ac in self.agents.items():
                            ...
                            ray.get(
                                ac.store_episode_length.remote(
    ...
    def process_render(
        self, epoch_ended: bool, epoch: int, logger: EpochLogger
    ) -> None:
