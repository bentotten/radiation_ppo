...
class AgentPPO:
    ...
    def __post_init__(self):
        ...
        if self.steps_per_epoch > 0:
            self.ppo_buffer = PPOBuffer.remote(
    ...
    def update_agent(
        ...
        if PRIO_MEMORY:
            ...
            actor_maps_buffer = ray.get(
                self.ppo_buffer.get_heatmap_buffer.remote("actor")
            )
            critic_maps_buffer = ray.get(
                self.ppo_buffer.get_heatmap_buffer.remote("critic")
            )
        ...
        if not self.GlobalCriticOptimizer or self.id == 0:
            for _ in range(self.train_v_iters):
                ...
                critic_loss_results = self.compute_batched_losses_critic(
                    data=data, sample=sample_indexes, map_buffer_maps=critic_maps_buffer
                )
    ...
    def update_rada2c(
        ...
        for ep in ep_form:
            ...
            val_loss = ray.get(
                self.agent_optimizer.MSE_loss.remote(val, ret)
            )  # MSE critc loss
    ...
    def store_episode_length(self, episode_length):
        ray.get(
            self.ppo_buffer.store_episode_length.remote(episode_length=episode_length)
        )
